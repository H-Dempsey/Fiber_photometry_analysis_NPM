...
def choose_which_type_analysis(inputs):
    ...
    sg.theme("DarkTeal2")
    layout  = []
    layout += [[sg.Text("Choose how you want the analysis to be done. \n"+
                        "Choose 'Manually score videos' if you are using \n"+
                        "this for the first time.\n")],
               [sg.Button("Analyse one folder"), sg.Button("Analyse many folders")],
               [sg.Button('Import settings file'), sg.Button("Manually score videos")]]
...
def analyse_many_folders(grouped_data, inputs):
    ...
    info['Baseline period'] = ('If "changing" is the baseline period type, fill in the \n'+
        ' secs before the event (e.g. -5, -1). If "constant" is the baseline period type, \n'+
        ' fill in the secs since the NPM box was turned on (e.g. 2000, 2100).')
...
def choose_name_NPM_event(inputs):
    ...
    layout += [[sg.T("")], [sg.Text("Choose an animal ID."), 
...
def choose_peri_event_options(inputs):
    ...
    layout = [[sg.T("")],
        ...
        [sg.Text("Choose the baseline period (secs)"), 
         sg.Combo(['Changing','Constant'],key="Baseline type",enable_events=True,
        ...
        [sg.Text("Save ISOS-GCaMP fit data to CSV?"), 
...
def choose_whole_recording_options(inputs):
    ...
    sg.theme("DarkTeal2")        
    layout = []
    layout += [[sg.T("")], [sg.Text("Choose how much data to\nexclude from the start (secs)"), 
    ...
    layout += [[sg.T("")], [sg.Text("Choose the baseline period"), 
                            sg.Combo(['Entire recording','Interval (secs)'],key="Baseline type",enable_events=True,
                                     default_value=default["Baseline type"], size=(13,1)),
                            sg.Input(key="BASELINE1",enable_events=True,visible=False,
                                     default_text=default["Baseline period"][0],size=(10,1)),
                            sg.Input(key="BASELINE2",enable_events=True,visible=False,
                                     default_text=default["Baseline period"][1],size=(10,1))]]
    layout += [[sg.T("")], [sg.Text("Smooth data using a moving average filter"),
                            sg.Input(key="Smoothing factor",enable_events=True,
                                     default_text=default["Smoothing factor"], size=(8,1))]]
    layout += [[sg.T("")], [sg.Text("Perform peak detection"), 
    ...
    layout += [[sg.T("")], [sg.Text("Create ISOS-GCaMP fit plot?"), 
                            sg.Combo(['True','False'],key="Fit",enable_events=True,
                                      default_value=default['Fit'])]]
    layout += [[sg.T("")], [sg.Button("Submit")]]
    window = sg.Window('Photometry Analysis', layout)
    while True:
        event, values = window.read()
        ...
        if values["Baseline type"] == "Interval (secs)":
...
def choose_peak_detection_options(inputs):
    ...
    sg.theme("DarkTeal2")
    layout  = []
    layout += [[sg.T("")], [sg.Text("Choose the minimum prominence for peak detection."),
                            sg.Input(key="Prominence",enable_events=True,
                                     default_text=default["Prominence"], size=(8,1))]]
    layout += [[sg.T("")], [sg.Text("Also include negative peaks?"), 
                            sg.Combo(['True','False'],key="Negative peaks",enable_events=True,
                                     default_value=default['Negative'])]]
    layout += [[sg.T("")], [sg.Button("Submit")]]
    window  = sg.Window('Photometry Analysis', layout)
    while True:
        event, values = window.read()
